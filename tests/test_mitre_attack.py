...
class TestMapFunctionToTechniques:
    def test_modbus_read_returns_collection(self):
        ...
        assert "T0802" in tech_ids  # Automated Collection

    def test_s7comm_writevar_returns_impair_process_control(self):
        techs = map_function_to_techniques("s7comm", "WriteVar")
        tech_ids = [t.technique_id for t in techs]
        assert "T0836" in tech_ids  # Modify Parameter
    ...
    def test_s7comm_stop_multiple_tactics(self):
        techs = map_function_to_techniques("s7comm", "Stop")
        tech_ids = {t.technique_id for t in techs}
        assert "T0858" in tech_ids  # Change Operating Mode
        assert "T0816" in tech_ids  # Device Restart/Shutdown

    def test_dnp3_operate_returns_impact(self):
        techs = map_function_to_techniques("dnp3", "Operate")
        tech_ids = [t.technique_id for t in techs]
        assert "T0821" in tech_ids  # Modify Controller Tasking
        assert "T0831" in tech_ids  # Manipulation of Control
...
class TestEnrichReportWithAttack:
    def test_s7comm_report_enriched(self):
        ...
        assert "T0836" in [t["technique_id"] for t in enriched["results"][0]["mitre_attack"]]
